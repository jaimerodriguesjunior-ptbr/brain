...
- A causa de o Modo Maquininha Pix piscar e fechar no tablet foi confirmada: o menu tablet não exige sessão do dashboard, mas a página e a API da maquininha exigiam. Localmente, o fluxo agora abre uma porta de PIN para funcionário ativo e grava um cookie HTTP-only, assinado, vinculado à loja e válido por oito horas. A página e a API aceitam essa autorização temporária sem reduzir o acesso existente do desktop. Typecheck e o teste de contrato Pix passaram; a mudança ainda não foi enviada ao app.
...
6. Publicar a porta de PIN da maquininha e validar no tablet da mesma loja a exibição de um QR Code gerado há menos de 30 minutos, além do aviso de falha caso a API não possa carregar a fila. Consumo baixo.
